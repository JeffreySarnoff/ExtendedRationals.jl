using XRationals
using Chairmarks

function fmt_ns(t)
    ns = minimum(t).time * 1e9
    if ns < 1000
        return "$(round(Int, ns)) ns"
    else
        return "$(round(ns/1000, digits=1)) μs"
    end
end

function run_table(label, R, make_r, make_qx)
    a_r, b_r = make_r(7, 3), make_r(5, 11)
    c_r, d_r = make_r(3, 13), make_r(11, 7)
    a_x, b_x = make_qx(7, 3), make_qx(5, 11)
    c_x, d_x = make_qx(3, 13), make_qx(11, 7)

    big_x = make_qx(typemax(R) - 1, 3)

    ops = []

    push!(ops, ("construct(7,3)",
        @be(make_r(7, 3)),
        @be(make_qx(7, 3))))

    push!(ops, ("a + b",
        @be($a_r + $b_r),
        @be($a_x + $b_x)))

    push!(ops, ("a - b",
        @be($a_r - $b_r),
        @be($a_x - $b_x)))

    push!(ops, ("a * b",
        @be($a_r * $b_r),
        @be($a_x * $b_x)))

    push!(ops, ("a / b",
        @be($a_r / $b_r),
        @be($a_x / $b_x)))

    push!(ops, ("-a",
        @be(-$a_r),
        @be(-$a_x)))

    push!(ops, ("a < b",
        @be($a_r < $b_r),
        @be($a_x < $b_x)))

    push!(ops, ("a == b",
        @be($a_r == $b_r),
        @be($a_x == $b_x)))

    push!(ops, ("abs(-a)",
        @be(abs(-$a_r)),
        @be(abs(-$a_x))))

    push!(ops, ("inv(a)",
        @be(inv($a_r)),
        @be(inv($a_x))))

    push!(ops, ("a ^ 3",
        @be($a_r ^ 3),
        @be($a_x ^ 3)))

    push!(ops, ("a+b+c+d",
        @be($a_r + $b_r + $c_r + $d_r),
        @be($a_x + $b_x + $c_x + $d_x)))

    push!(ops, ("a*b-c*d",
        @be($a_r * $b_r - $c_r * $d_r),
        @be($a_x * $b_x - $c_x * $d_x)))

    push!(ops, ("muladd(a,b,a)",
        @be(muladd($a_r, $b_r, $a_r)),
        @be(muladd($a_x, $b_x, $a_x))))

    push!(ops, ("fma(a,b,a)",
        @be(fma($a_r, $b_r, $a_r)),
        @be(fma($a_x, $b_x, $a_x))))

    push!(ops, ("big + big",
        nothing,
        @be($big_x + $big_x)))

    inf_x = make_qx(1, 0)
    push!(ops, ("Inf + a",
        nothing,
        @be($inf_x + $a_x)))

    r_label = "`Rational{$(R)}`"
    qx_label = R == Int32 ? "`Qx32`" : "`Qx64`"

    println("\n### $label\n")
    println("| Operation | $r_label | $qx_label | Speedup |")
    println("| --- | --- | --- | --- |")

    for (name, tr, tx) in ops
        r_str = tr === nothing ? "---" : fmt_ns(tr)
        x_str = fmt_ns(tx)
        if tr === nothing
            spd_str = "---"
        else
            r_ns = minimum(tr).time * 1e9
            x_ns = minimum(tx).time * 1e9
            spd = r_ns / x_ns
            spd_str = spd >= 1.05 ? "$(round(spd, digits=1))x" : (spd <= 0.95 ? "$(round(spd, digits=2))x" : "~1x")
        end
        println("| $name | $r_str | $x_str | $spd_str |")
    end
end

# --- 32-bit table ---

run_table(
    "32-bit",
    Int32,
    (n, d) -> Rational{Int32}(Int32(n), Int32(d)),
    (n, d) -> Qx32(n, d)
)

# --- 64-bit table ---

run_table(
    "64-bit",
    Int64,
    (n, d) -> Rational{Int64}(Int64(n), Int64(d)),
    (n, d) -> Qx64(n, d)
)

# --- Qx64 vs Rational{Int64} speedup table ---

function run_speedup_table()
    make_r = (n, d) -> Rational{Int64}(Int64(n), Int64(d))
    make_x = (n, d) -> Qx64(n, d)

    a_r, b_r = make_r(7, 3), make_r(5, 11)
    c_r, d_r = make_r(3, 13), make_r(11, 7)
    a_x, b_x = make_x(7, 3), make_x(5, 11)
    c_x, d_x = make_x(3, 13), make_x(11, 7)

    ops = [
        ("construct(7,3)", @be(make_r(7, 3)),           @be(make_x(7, 3))),
        ("a + b",          @be($a_r + $b_r),            @be($a_x + $b_x)),
        ("a - b",          @be($a_r - $b_r),            @be($a_x - $b_x)),
        ("a * b",          @be($a_r * $b_r),            @be($a_x * $b_x)),
        ("a / b",          @be($a_r / $b_r),            @be($a_x / $b_x)),
        ("-a",             @be(-$a_r),                   @be(-$a_x)),
        ("a < b",          @be($a_r < $b_r),            @be($a_x < $b_x)),
        ("a == b",         @be($a_r == $b_r),           @be($a_x == $b_x)),
        ("abs(-a)",        @be(abs(-$a_r)),              @be(abs(-$a_x))),
        ("inv(a)",         @be(inv($a_r)),               @be(inv($a_x))),
        ("a ^ 3",          @be($a_r ^ 3),               @be($a_x ^ 3)),
        ("a+b+c+d",        @be($a_r+$b_r+$c_r+$d_r),   @be($a_x+$b_x+$c_x+$d_x)),
        ("a*b-c*d",        @be($a_r*$b_r-$c_r*$d_r),   @be($a_x*$b_x-$c_x*$d_x)),
        ("muladd(a,b,a)",  @be(muladd($a_r,$b_r,$a_r)), @be(muladd($a_x,$b_x,$a_x))),
        ("fma(a,b,a)",     @be(fma($a_r,$b_r,$a_r)),    @be(fma($a_x,$b_x,$a_x))),
    ]

    println("\n### Qx64 vs Rational{Int64} Speedup\n")
    println("`Qx64` delays GCD normalization until display, hashing, or conversion, giving IEEE-like Inf/NaN semantics and faster arithmetic than stdlib.\n")
    println("| Operation | `Rational{Int64}` | `Qx64` | Speedup |")
    println("| --- | --- | --- | --- |")

    for (name, tr, tx) in ops
        r_ns = minimum(tr).time * 1e9
        x_ns = minimum(tx).time * 1e9
        spd = r_ns / x_ns
        spd_str = spd >= 1.05 ? "$(round(spd, digits=1))x" : (spd <= 0.95 ? "$(round(spd, digits=2))x" : "~1x")
        println("| $name | $(fmt_ns(tr)) | $(fmt_ns(tx)) | $spd_str |")
    end
end

run_speedup_table()
