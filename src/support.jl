...
@inline function _checked_int256(x::Integer)
    typemin(Int256) <= x <= typemax(Int256) || throw(OverflowError("value does not fit in Int256"))
    return Int256(x)
end
...
@inline _apply_sign(x::Rational{Int256}, negative::Bool) = negative ? -x : x
...
@inline function _tie_even(a::Rational{Int256}, b::Rational{Int256})
    a_even = iseven(numerator(a))
    b_even = iseven(numerator(b))
    if a_even != b_even
        return a_even ? a : b
...
function _compare_distance(target::Rational{Int256}, a::Rational{Int32}, b::Rational{Int32})
...
function _compare_distance(target::Rational{Int512}, a::Rational{Int32}, b::Rational{Int32})
    tn = Int1024(numerator(target))
    td = Int1024(denominator(target))
...
function _compare_distance(target::Rational{Int512}, a::Rational{Int64}, b::Rational{Int64})
    tn = Int1024(numerator(target))
    td = Int1024(denominator(target))
...
function _compare_distance(target::Rational{Int1024}, a::Rational{Int256}, b::Rational{Int256})
    tn = Int2048(numerator(target))
    td = Int2048(denominator(target))

    an = abs(tn * Int2048(denominator(a)) - Int2048(numerator(a)) * td)
    bn = abs(tn * Int2048(denominator(b)) - Int2048(numerator(b)) * td)

    lhs = an * Int2048(denominator(b))
    rhs = bn * Int2048(denominator(a))
...
function _nearest_rational32(target::Rational{Int256})
    iszero(target) && return zero(Rational{Int32})

    limit = Int256(typemax(Int32))
    negative = target < 0
    work = negative ? -target : target
...
function _nearest_rational32(target::Rational{Int512})
    iszero(target) && return zero(Rational{Int32})

    limit = Int512(typemax(Int32))
    negative = target < 0
    work = negative ? -target : target
...
function _nearest_rational64(target::Rational{Int512})
    iszero(target) && return zero(Rational{Int64})

    limit = Int512(typemax(Int64))
    negative = target < 0
    work = negative ? -target : target
...
function _nearest_rational256(target::Rational{Int1024})
    iszero(target) && return zero(Rational{Int256})

    limit = Int1024(typemax(Int256))
    negative = target < 0
    work = negative ? -target : target

    if numerator(work) > limit * denominator(work)
        return _apply_sign(_checked_int256(limit) // Int256(1), negative)
    end

    if numerator(work) <= limit && denominator(work) <= limit
        exact = _checked_int256(numerator(work)) // _checked_int256(denominator(work))
        return _apply_sign(exact, negative)
    end
    ...
    while true
        ...
        if p2 > limit || q2 > limit
            ...
            lower = _checked_int256(p0 + k * p1) // _checked_int256(q0 + k * q1)
            upper = _checked_int256(p1) // _checked_int256(q1)
        ...
        if rem(n, d) == 0
            exact = _checked_int256(p2) // _checked_int256(q2)
            return _apply_sign(exact, negative)
        end
